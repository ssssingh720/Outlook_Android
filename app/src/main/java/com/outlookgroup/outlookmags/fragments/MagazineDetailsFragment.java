    ...
    private View loadCardItem(int position, View view, Card data) {
        ...
        if (data.getImage() != null && !data.getImage().isEmpty()) {
            if(position == 0) {
                ...
                Picasso.with(getActivity()).load(data.getImage()).resize(Util.dipToPixels(getActivity(),100),Util.dipToPixels(getActivity(),100)).into(userImg);
            }
        } else {
            coverImg.setVisibility(View.GONE);
            userImg.setVisibility(View.GONE);
        ...
        if (isPurchased || data.getPaid()) {
            ...
            // hiding lock image : Make visible once payment is open
            blockImg.setVisibility(View.GONE);
    ...
    public void onClick(View v) {
        ...
        if (tag!=null) {
            ...
            if (!magazineID.equals("5")){
                //====== commenting subscribe content: uncomment it once Payment is OPen ====
            //showPopUpSubscribe();
            }
            else{
                // buy
